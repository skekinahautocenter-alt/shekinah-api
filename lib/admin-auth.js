...
module.exports = function registerAdmin({ app, pool, adminPassword, sessionSecret, json }) {
  ...
  async function credentials() {
    ...
    if (!result.rows[0]) {
      if (typeof adminPassword !== 'string' || !adminPassword.trim() || adminPassword.length > 256) throw new Error('Missing bootstrap password');
  ...
  app.put('/api/admin/password', requireAdmin, json, async (req, res) => {
    try {
      ...
      if (typeof newPassword !== 'string' || newPassword.length > 256 || !newPassword.trim()) return res.status(400).json({ error: 'Informe uma nova senha não vazia, com até 256 caracteres.' });
