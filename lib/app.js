...
function createApp({ pool, adminPassword, sessionSecret, trustProxy = false }) {
  ...
  app.get('/api/banner', async (req, res) => {
    ...
    try {
      const result = await pool.query('SELECT alt_text, version, updated_at, height FROM site_banner WHERE id = 1');
  ...
  app.put('/api/banner', requireAdmin, express.json({ limit: '3mb' }), async (req, res) => {
    ...
    let image;
    try {
      const input = sharp(source, { limitInputPixels: 1080 * 1350, failOn: 'warning' });
      const metadata = await input.metadata();
      if (metadata.format !== match[1] || (metadata.pages || 1) !== 1 || metadata.width !== 1080 || metadata.height !== 1350) {
        return res.status(400).json({ error: 'A arte deve ser uma imagem estática de exatamente 1080 × 1350 pixels.' });
      }
      // Fully decode and re-encode: reject corrupt files and discard embedded metadata.
      image = await input.webp({ quality: 90 }).toBuffer();
    } catch {
      return res.status(400).json({ error: 'Não foi possível ler a arte. Use uma imagem válida de 1080 × 1350 pixels.' });
    }
    try {
      const result = await pool.query(`INSERT INTO site_banner (id, image, alt_text, version, height) VALUES (1, $1, $2, $3, 1350)
        ON CONFLICT (id) DO UPDATE SET height = EXCLUDED.height, image = EXCLUDED.image, alt_text = EXCLUDED.alt_text, version = EXCLUDED.version, updated_at = CURRENT_TIMESTAMP
        RETURNING alt_text, version, updated_at, height`, [image, altText.trim(), crypto.randomUUID()]);
      res.json({ banner: describe(result.rows[0]) });
    } catch {
